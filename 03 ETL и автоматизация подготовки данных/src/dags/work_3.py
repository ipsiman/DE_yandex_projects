...
def get_report(ti):
    ...
    if not report_id:
        raise TimeoutError('Error getting report.')
...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    conn = engine.connect()
    sql_text = f"DELETE FROM staging.user_order_log WHERE date_time::Date = '{date}';"
...
def start_update_facts():
    task_logger.info('Start update facts')
...
with DAG(
    ...
    start_update_facts = PythonOperator(
        task_id='start_update_facts',
        python_callable=start_update_facts)
